...
using Foundation.AspNetCore.Features.MyAccount.OrderConfirmation.Controllers;
using Foundation.AspNetCore.Features.MyAccount.OrderHistory.Models;
...
namespace Foundation.AspNetCore.Features.MyAccount.OrderHistory.Controllers
{
    [Authorize]
    public class OrderHistoryController : OrderConfirmationControllerBase<OrderHistoryPage>
    {
        ...
        public IActionResult Index(OrderHistoryPage currentPage, OrderFilter filter, int? page, int? size, int? isPaging)
        {
        ...
        public IActionResult ViewAll() => Redirect(UrlResolver.Current.GetUrl(_settingsService.GetSiteSettings<ReferencePageSettings>()?.OrderHistoryPage ?? ContentReference.StartPage));

        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult SaveAsPaymentPlan(int orderid, int cycleMode, int cycleLength)
        {
